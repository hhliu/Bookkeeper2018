...
public class DB_s {
	...
	private static final String TAG = DB_s.class.getSimpleName();
	...
	/** 開啟寫入模式（可寫可讀） */
	public DB_s openToWrite() throws SQLException {
		synchronized (LOCK) {
	...
	/** 開啟讀取模式 */
	public DB_s openToRead() throws SQLException {
		synchronized (LOCK) {
	...
	public void update(int id, ContentValues values) throws SQLException {
		synchronized (LOCK) {
			db.update(TABLE, values, DB_s.KEY_ID + "=" + id, null);
		}
	}
	...
	public void delete(int id) throws SQLException {
		synchronized (LOCK) {
			db.delete(TABLE, DB_s.KEY_ID + "=" + id, null);
		}
	}
